...
export function validateCsrfToken(req) {
    ...
    // ── Double-submit check (timing-safe) ───────────────────────────────────
    // ใช้ timingSafeEqual แทน !== เพื่อป้องกัน timing oracle:
    //   ถ้า cookie เป็น httpOnly ในอนาคต attacker ไม่รู้ค่า cookie
    //   → วัด timing ของ !== ได้ว่า byte ใดที่ตรงกัน → reconstruct token ทีละ byte
    //   timingSafeEqual: constant-time สำหรับ buffer ยาวเท่ากัน
    //   length ต่างกัน → return false ทันที (ไม่ leak ข้อมูลว่า prefix ตรงกี่ตัว)
    //
    // สำคัญ: ต้อง encode เป็น Buffer ก่อนตรวจ length ไม่ใช่ string.length
    //   multi-byte UTF-8 chars (เช่น emoji, Thai) ทำให้ byteLength > string.length
    //   timingSafeEqual() throw RangeError ถ้า buffer สองตัวยาวไม่เท่ากัน
    //   → ต้อง check buf.length (ไม่ใช่ str.length) ก่อนเรียก timingSafeEqual
    //   Token ของเราเป็น hex ล้วน (ASCII) จึงไม่มี multi-byte จริง
    //   แต่ทำ byte-level check เป็น defense-in-depth รองรับ implementation เปลี่ยนในอนาคต
