...
//
//   IPv4-mapped IPv6 — RFC 4291 กำหนด 2 format:
//     Format A: ::ffff:A.B.C.D      (standard, Node.js ใช้รูปนี้)
//     Format B: ::ffff:0:A.B.C.D   (alternative — บาง OS/middleware ใช้)
//     ต้อง cover ทั้งสองรูปสำหรับทุก private range:
//       ::ffff:127.x.x.x / ::ffff:0:127.x.x.x    (loopback mapped)
//       ::ffff:10.x.x.x  / ::ffff:0:10.x.x.x     (RFC 1918 /8 mapped)
//       ::ffff:192.168.x / ::ffff:0:192.168.x     (RFC 1918 /16 mapped)
//       ::ffff:172.1x.x  / ::ffff:0:172.1x.x      (RFC 1918 /12 mapped)
//       ::ffff:169.254.x / ::ffff:0:169.254.x      (link-local mapped)
//     ถ้าขาด Format B: proxy ที่ใช้ alternative representation จะถูก classify
//     ผิดเป็น "direct connection" → ใช้ socket IP (proxy IP) เป็น rate-limit key
//     → ทุก client ผ่าน proxy นั้น share bucket เดียว → collateral DoS
const PRIVATE_IP_REGEX = /^(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.|127\.|169\.254\.|::1$|::ffff:127\.|::ffff:10\.|::ffff:192\.168\.|::ffff:172\.(1[6-9]|2\d|3[01])\.|::ffff:169\.254\.|::ffff:0:127\.|::ffff:0:10\.|::ffff:0:192\.168\.|::ffff:0:172\.(1[6-9]|2\d|3[01])\.|::ffff:0:169\.254\.|[fF][cCdD][0-9a-fA-F]{2}:|[fF][eE][89aAbB][0-9a-fA-F]:)/;
