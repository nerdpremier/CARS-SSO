...
export function hashMfaCode(code, logId) {
    ...
    // ตรวจรูปแบบ logId: ต้องเป็น digit ล้วน (DB BIGSERIAL ID)
    // ป้องกัน "1e5" (scientific notation): Number("1e5") === 100000 แต่ "1e5" ไม่ใช่ plain ID
    // ป้องกัน UUID / non-numeric string ที่ไม่มีทาง match กับ ID ใน DB
    const LOGID_PATTERN = /^\d+$/;
    if (!LOGID_PATTERN.test(logIdStr)) {
        throw new Error('[INPUT] logId must be a non-empty numeric string (DB integer ID)');
    }
