...
export async function blockUser(username, ip) {
    ...
    try {
        ...
        // Delete existing then insert (avoid ON CONFLICT issue)
        ...
        auditLog('USER_BLOCKED', { username, ip, durationMinutes: 1 });
        console.log(`[INFO] User ${username} blocked for 1 minute${ip ? ' (IP: ' + ip + ')' : ''}`);
    } catch (err) {
