...
export async function checkRateLimit(key, maxCount, windowMs) {
    ...
    // max 256 chars: "ip:xxxx:route" ยาวสุดไม่เกิน ~60 chars ในทางปฏิบัติ
    // key ยาวเกิน → DB index bloat + write amplification โดยไม่จำเป็น
    if (!key || typeof key !== 'string') {
        // key ไม่ถูก log เพื่อป้องกัน user-controlled value รั่วใน error message / log
        throw new Error('[rate-limit] invalid key: must be a non-empty string');
    }
    if (key.length > 256) {
        throw new Error(`[rate-limit] key too long (${key.length} chars, max 256)`);
    }
    ...
    // แม้ cleanup trigger ไม่ใช่ security decision แต่ consistency ลด cognitive overhead
    //
