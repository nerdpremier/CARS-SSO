...
export async function blockUser(username, ip) {
    if (!username || typeof username !== 'string') {
        console.error('[WARN] blockUser called with invalid username:', username);
        return;
    }
    ...
    try {
        ...
        await pool.query('DELETE FROM rate_limit_events WHERE key = $1', [key]);
        await pool.query(
        ...
        if (ip && typeof ip === 'string') {
            ...
            await pool.query('DELETE FROM rate_limit_events WHERE key = $1', [ipKey]);
            await pool.query(
                'INSERT INTO rate_limit_events (key, created_at) VALUES ($1, NOW())',
                [ipKey]
            );
        ...
        auditLog('USER_BLOCKED', { username, ip, durationSeconds: 60 });
        console.log(`[INFO] User ${username} blocked for 60 seconds${ip ? ' (IP: ' + ip + ')' : ''}`);
    } catch (err) {
        console.error('[WARN] blockUser failed:', err.message);
        auditLog('BLOCK_USER_FAILED', { username, ip, error: err.message });
    }
}
...
export async function isUserBlocked(username, ip) {
    try {
        ...
        if (userResult.rows.length > 0) {
            ...
            blockedUntil.setSeconds(blockedUntil.getSeconds() + 60);
