...
export async function ensureBehaviorRisksSchema() {
    ...
    try {
        await pool.query(`ALTER TABLE oauth_codes ADD COLUMN IF NOT EXISTS pre_login_log_id INTEGER`);
    } catch { }
    try {
        await pool.query(`ALTER TABLE oauth_tokens ADD COLUMN IF NOT EXISTS pre_login_log_id INTEGER`);
    } catch { }
    try {
