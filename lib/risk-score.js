...
export function getCombinedConfig() {
    // Default weights (edit in code if you don't want env-based config)
    const DEFAULT_W1 = 0.3;
    const DEFAULT_W2 = 0.65;

    const w1 = Number(process.env.RISK_W1 ?? String(DEFAULT_W1));
    const w2 = Number(process.env.RISK_W2 ?? String(DEFAULT_W2));
    const medium = Number(process.env.RISK_COMBINED_MEDIUM_THRESHOLD ?? '0.65');
    const revoke = Number(process.env.RISK_COMBINED_REVOKE_THRESHOLD ?? '1');
    return {
        w1: Number.isFinite(w1) ? w1 : DEFAULT_W1,
        w2: Number.isFinite(w2) ? w2 : DEFAULT_W2,
        medium: Number.isFinite(medium) ? medium : 0.65,
        revoke: Number.isFinite(revoke) ? revoke : 1,
    };
}
